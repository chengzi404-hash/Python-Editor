...
**Location**: [`ui/widgets/`](../../ui/widgets/)
...
[`ui/widgets/__init__.py`](../../ui/widgets/__init__.py).
...
| Class | File | Notes |
| --- | --- | --- |
| `Theme` | `theme.py` | Base class with class-level color / font constants. |
| `DarkTheme` | `theme.py` | The default `name='Dark'`. |
| `LightTheme` | `theme.py` | `name='Light'`. |
| `SolarizedDarkTheme` | `theme.py` | `name='Solarized Dark'`. |

`Theme` attributes include `BG_TITLE`, `BG_BASE`, `BG_PANEL`,
`BG_RAISED`, `BG_HOVER`, `BG_ACTIVE`, `BG_INPUT`, `BG_DROPDOWN`,
`FG_PRIMARY`, `FG_SECONDARY`, `FG_TERTIARY`, `FG_DISABLED`, `BORDER`,
`BORDER_STRONG`, `BORDER_FOCUS`, `RED`/`RED_HOVER`/`RED_DARK`,
`YELLOW_*`, `GREEN_*`, `BLUE_*`, `PURPLE`, `TITLE_ACCENT_WIDTH`,
`TITLE_ACCENT`, `TITLE_FONT`, `MENU_FONT`, `LABEL_FONT`,
`LABEL_FONT_BOLD`, `LABEL_FONT_SMALL`, `BUTTON_FONT`, `ICON_FONT`,
`MONO_FONT`.
...
### `UFrame(parent, *, variant='base', **kw)`

`tk.Frame` subclass with `variant` ∈ `{'title', 'base', 'panel',
'raised', 'input'}`. `_apply_theme()` rebuilds colors when the theme
changes.

### `ULabel(parent, text='', *, variant='primary', **kw)`

`tk.Label` subclass with `variant` ∈ `{'primary', 'secondary',
'tertiary', 'disabled', 'blue', 'red', 'green', 'yellow'}`.

### `UButton(parent, text='', *, command=None, variant='default', state='normal', width=None, **kw)`

Canvas-drawn rounded button (not `tk.Button`). `variant` ∈ `{'default',
'primary', 'success', 'danger', 'warning', 'ghost'}`. `state` ∈
`{'normal', 'disabled'}`.

### `UEntry(parent, *, textvariable=None, placeholder='', width=20, show='', **kw)`

`tk.Frame` containing `tk.Entry` with placeholder + theme-aware
colors. `show` enables password masking.

### `UText(parent, *, height=10, width=40, line_numbers=False, **kw)`

`tk.Frame` wrapping `tk.Text` + `UScrollBar` (optional
`LineNumberCanvas`). Methods:
...
### `UCheckButton(parent, *, text='', variable=None, command=None, **kw)`

Custom-drawn check button.

### `URadioButton(parent, *, text='', variable=None, value=None, command=None, **kw)`

Custom-drawn radio button.

### `UComboBox(parent, *, values=(), variable=None, command=None, width=20, **kw)`

Custom dropdown built on a `tk.Toplevel`. Use like
`tkinter.ttk.Combobox`.

### `UProgressBar(parent, *, maximum=100, value=0, orient='horizontal', **kw)`

`tk.Canvas`-drawn progress bar.

### `USlider(parent, *, minimum=0, maximum=100, value=0, orient='horizontal', command=None, **kw)`

`tk.Canvas`-drawn slider, horizontal or vertical.

### `UScrollBar(parent, *, orient='vertical', autohidden=False, **kw)`
...
### `UMenu(parent, **kw)` / `UMenuBar(parent, **kw)`
...
| Method | Description |
| --- | --- |
| `add_command(label, command, shortcut=None)` | Add a clickable item. |
| `add_separator()` | Add a visual separator. |
| `add_radiobutton(label, value, variable)` | Add a radio-style item. |
| `add_checkbutton(label, variable)` | Add a check-style item. |
| `add_cascade(label, submenu=None)` | Add a submenu (used by `UMenuBar.add_cascade`). |
...
### `UEditorSuggestion(parent, items=(), on_select=None, *, max_visible=8, show_detail=True, show_description=True, grab_focus=False)`
...
### `TabBar(parent, *, on_select=None, on_close=None, on_context_menu=None, **kw)`
...
### `UDialog(parent, *, title='', width=400, height=240)`
...
### `UListView(parent, *, columns=(), on_select=None)`
...
## Message boxes (`ui.widgets.message_box`)
...
### `ActivityBarItem(parent, *, icon='explorer', tooltip='', on_click=None)`
...
Full Git UI: branch chips, ahead/behind counts, commit composer
(Ctrl+Enter), staged / unstaged lists, push / pull / refresh / commit
(Amend).
...
Re-exported through `ui.widgets`. Same shape as the highlight and
language marketplaces:

```python
from ui.widgets import ui_theme_marketplace
...
```python
from ui.widgets.icons import ICON_SIZE, draw_icon
...
`__all__ = ["ActivityBar", "ActivityBarItem", "CompletionItem",
"DebugCard", "ExplorerCard", "GitCard", "LineNumberCanvas",
"NavSelection", "SideBar", "Tab", "TabBar", "TreeCanvas", "UButton",
"UCheckButton", "UComboBox", "UDialog", "UEditorSuggestion", "UEntry",
"UFileTree", "UFrame", "ULabel", "UListView", "UMenu", "UMenuBar",
"UProgressBar", "URadioButton", "UScrollBar", "USettingsNavBar",
"USlider", "UTabView", "UText", "message_box", "theme",
"ui_theme_marketplace"]`.