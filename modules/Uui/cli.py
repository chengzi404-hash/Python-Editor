...
def _prompt(question: str, default: str = '', *,
            required: bool = False, validator=None) -> str:
    suffix = f' [{default}]' if default else ''
    while True:
        try:
            raw = input('  ' + _c(_C.YELLOW, '? ') + question + _c(_C.DIM, suffix) + _c(_C.YELLOW, ' › '))
        except EOFError:
            return default
        ans = raw.strip()
        if not ans:
...
def _prompt_choice(question: str, choices: list, default=None,
                   *, index_from: int = 1) -> str:
    if default is None:
        default = choices[0]
    ...
    while True:
        try:
            raw = input('  ' + _c(_C.YELLOW, '  choice') + _c(_C.DIM, f' [{default}]') + _c(_C.YELLOW, ' › '))
        except EOFError:
            return default
        ans = raw.strip()
        if not ans:
...
def _prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    while True:
        try:
            raw = input('  ' + _c(_C.YELLOW, '? ') + question + _c(_C.DIM, f' {suffix}') + _c(_C.YELLOW, ' › '))
        except EOFError:
            return default
        ans = raw.strip().lower()
        if not ans:
...
def _detect_python_envs() -> list:
    ...
    for name in ('python', 'python3', 'py'):
        ...
        if not ver:
            continue
        seen.add(resolved)
        label = f'venv  (python {ver})' if name == 'python' \
               else f'venv via {name}  (python {ver})'
        found.append({
            'kind': 'venv',
            'path': resolved,
            'label': label,
...
def _init_git(project_dir: str) -> None:
    ...
    try:
        ...
        subprocess.run(['git', 'add', '.'], cwd=project_dir,
                       check=True, capture_output=True)
        subprocess.run(
            ['git', '-c', 'user.email=uui@local',
             '-c', 'user.name=Uui Scaffold',
             'commit', '-q', '-m', 'Initial scaffold from Uui'],
...
def _init_svn(project_dir: str) -> None:
    ...
    print('    ' + _c(_C.CYAN,
        f'svn checkout {repo_url}/trunk {project_dir}-wc'))
...
def _new_interactive(args: argparse.Namespace) -> int:
    ...
    project = _prompt('Project name (a Python identifier)',
    ...
    default_theme = _prompt_choice('Default Uui theme', available_themes,
                                    default=cfg_default_theme)

    _step(4, total_steps, 'Window geometry')
    geometry = _prompt('Window geometry', default=cfg_geometry,
                        validator=_validate_geometry)

    _step(5, total_steps, 'OS theme follow')
    follow_os = _prompt_yes_no('Follow OS theme at runtime?',
                                default=cfg_follow_os)
    ...
    if python_envs:
        ...
        picked = _prompt_choice(
            ...
            env_labels, default=env_default_label,
        )
        if picked != 'none':
    ...
    env_display = (env_choice['short']
                    if env_choice else 'none')
...
def _new_apply(*, name: str, title: str, theme: str, geometry: str,
               follow_os: bool, env_choice, vcs: str,
               force: bool, silent: bool) -> int:
    cfg = _load_config() or type('_Defaults', (), {})()
    preload = getattr(cfg, 'PRELOAD', '\n') or '\n'
    os_map = getattr(cfg, 'FOLLOW_SYSTEM_THEME',
                     {'dark': 'Dark', 'light': 'Light', 'default': 'Dark'})
    ...
    if silent:
        ...
        if env_choice is not None and env_choice['kind'] == 'venv':
            ...
            print('    ' + _c(_C.CYAN,
                f'conda activate {os.path.abspath(os.path.join(name, ".venv"))}'))
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p_new.add_argument('--force', action='store_true',
                        help='overwrite files when they exist')
    ...
    p_theme.add_argument('--force', action='store_true',
    ...
    p_cfg.add_argument('--init', action='store_true',
                        help='create config if missing')
    p_cfg.add_argument('--reset', action='store_true',
                        help='overwrite config with defaults')
    ...
    p_web.add_argument('web_args', nargs=argparse.REMAINDER,
                        help='arguments forwarded to `Uui.web.cli`')
