...
def _build_header(window: Window) -> None:
    ...
    ULabel(title_row, text='Uui  Component  Gallery',
            font=H1, variant='primary').pack(side=tk.LEFT, anchor=tk.W)
...
def _build_theme_picker(parent, window: Window) -> UFrame:
    ...
    ULabel(picker, text='Theme', variant='secondary',
            font=theme.LABEL_FONT_SMALL).pack(side=tk.LEFT, padx=(0, 8))
...
def _build_button_row(canvas, body) -> None:
    ...
    UButton(row, text='Disabled', command=lambda: None,
             variant='primary', state='disabled').pack(side=tk.LEFT, padx=4, pady=4)

    ULabel(body, text='Click any button to print its variant to the console.',
            variant='tertiary', font=theme.LABEL_FONT_SMALL).pack(anchor=tk.W, pady=(0, 18))
...
def _build_inputs(canvas, body) -> None:
    ...
    UEntry(grid, show='\u2022', placeholder='At least 8 chars').grid(row=2, column=1, sticky=tk.EW, pady=6)
...
def _build_progress_and_slider(canvas, body) -> None:
    ...
    progress_label = ULabel(
        wrap, text='Loading assets... 35%',
        variant='secondary', font=theme.LABEL_FONT_SMALL,
    )
    ...
    ULabel(
        wrap, text='Drag the slider — the progress bar above updates live.',
        variant='tertiary', font=theme.LABEL_FONT_SMALL,
