...
WIDGET_GROUPS = [
    ('Layouts',      []),
    ('Spacers',      []),
    ('Buttons',      [('UButton',      'PushButton',  '▶'),
                      ('UCheckButton', 'CheckBox',    '☑'),
                      ('URadioButton', 'RadioButton', '◉')]),
    ('Item Widgets', [('UComboBox',    'ComboBox',    '▼')]),
    ('Input Widgets',[('UEntry',       'LineEdit',    '|'),
                      ('UText',        'TextEdit',    '¶')]),
    ('Display',      [('ULabel',       'Label',       'T'),
...
TYPE_PROPS = {
    ...
    'UButton':      [('Text',  ['text', 'variant']),
                     ('Code',  ['command'])],
    'UEntry':       [('Input', ['placeholder', 'show'])],
    'UText':        [],
    'UCheckButton': [('Text',  ['text']),
                     ('Code',  ['command'])],
    'URadioButton': [('Text',  ['text']),
                     ('Code',  ['command'])],
    'UComboBox':    [('Items', ['values']),
                     ('Code',  ['command'])],
    'UProgressBar': [('Value', ['maximum', 'value', 'color'])],
    'USlider':      [('Range', ['from_', 'to', 'value', 'orient', 'show_value']),
                     ('Code',  ['command'])],
}
...
class _FlatButton(tk.Frame):
    ...
    def __init__(self, parent, text: str, command=None, width: int = 30,
                 tooltip: str = '', font=None, accent: bool = False):
        super().__init__(parent, bg=theme.BG_TITLE, highlightthickness=0, bd=0,
        ...
        self._label = tk.Label(
            self, text=text, bg=self._normal_bg, fg=theme.FG_PRIMARY,
            font=font or theme.MENU_FONT, cursor='hand2', padx=4,
        )
...
class _PanelSection(tk.Frame):
    ...
    def __init__(self, parent, title: str, expanded: bool = True):
        ...
        self._arrow = tk.Label(
            ...
            font=('Arial', 9), width=2, anchor='center',
        )
...
class _SidebarTabBar(tk.Frame):
    ...
    def __init__(self, parent, tabs, command=None):
        ...
        for idx, label in enumerate(tabs):
            btn = tk.Label(
                self, text=label, bg=theme.BG_TITLE,
...
class DesignerApp(Window):
    ...
    def _create_menu(self):
        ...
        form_menu.add_command('Rename...', lambda: self._rename_widget(self._selected_id) if self._selected_id else None, 'F2')
    ...
    def _create_toolbar(self):
        ...
        for item in actions:
            if item is None:
                ...
                continue
            glyph, cmd, tip = item
            btn = _FlatButton(self._toolbar, text=glyph, command=cmd, tooltip=tip,
        ...
        self._mode_label = tk.Label(
            self._toolbar, text='DESIGN', bg=theme.BG_TITLE,
            fg=theme.BLUE, font=('Arial', 9, 'bold'), padx=10,
        )
    ...
    def _create_body(self):
        ...
        sash_kw = {'sashrelief': str(tk.FLAT), 'sashwidth': 4, 'sashpad': 0,
                   'bg': str(theme.BORDER), 'bd': 0}
        try:
            self._left_paned = tk.PanedWindow(
                self._body, orient=tk.HORIZONTAL, **sash_kw,
            )
    ...
    def _build_left_sidebar(self):
        ...
        self._left_tabs = _SidebarTabBar(
            self._left_frame, ['Widget Box', 'Object Explorer'],
    ...
    def _build_widget_palette(self):
        ...
        self._palette_window_id = self._palette_canvas.create_window(
            (0, 0), window=self._palette_inner, anchor='nw',
        )
        self._palette_inner.bind(
            '<Configure>',
            lambda e: self._palette_canvas.config(
                scrollregion=self._palette_canvas.bbox('all')),
        )
        ...
        search_icon = tk.Label(filter_row, text='⌕', bg=theme.BG_PANEL,
                               fg=theme.FG_SECONDARY, font=('Arial', 11))
        ...
        filter_entry = UEntry(filter_row, textvariable=self._palette_filter,
    ...
    def _make_palette_cell(self, parent, wtype: str, friendly: str) -> tk.Frame:
        ...
        icon_box = tk.Label(
            ...
            font=('Arial', 11, 'bold'), width=2, height=1,
        )
        ...
        name_lbl = tk.Label(
            ...
            font=theme.LABEL_FONT_SMALL, anchor='w',
        )
        ...
        for w in (cell, icon_box, name_lbl):
            ...
            w.bind('<Enter>', lambda e, c=cell, n=name_lbl, i=icon_box: (
                c.config(bg=theme.BG_HOVER),
                n.config(bg=theme.BG_HOVER),
                i.config(bg=theme.BG_HOVER),
            ))
            w.bind('<Leave>', lambda e, c=cell, n=name_lbl, i=icon_box: (
    ...
    def _build_object_explorer(self):
        ...
        tk.Label(header, text='FORM OBJECTS',
                 bg=theme.BG_PANEL, fg=theme.FG_SECONDARY,
                 font=('Arial', 8, 'bold')).pack(side=tk.LEFT)
        ...
        self._explorer_window_id = self._explorer_canvas.create_window(
            (0, 0), window=self._explorer_inner, anchor='nw',
        )
        self._explorer_inner.bind(
            '<Configure>',
            lambda e: self._explorer_canvas.config(
                scrollregion=self._explorer_canvas.bbox('all')),
        )
    ...
    def _refresh_explorer(self):
        ...
        root_row = self._make_explorer_row(
            self._explorer_inner, 'Form', 'main',
            '\u25A2', is_header=True,
        )
        ...
        for item in self._widgets:
            row = self._make_explorer_row(
                self._explorer_inner, item['type'], item['name'],
    ...
    def _make_explorer_row(self, parent, kind: str, name: str, icon: str,
                           item_id=None, indent: int = 0, is_header: bool = False):
        bg = theme.BG_PANEL
        ...
        kind_lbl = tk.Label(
            row, text=kind,
            bg=row['bg'], fg=theme.FG_TERTIARY,
    ...
    def _build_center(self):
        ...
        self._theme_combo = UComboBox(
            bar, values=[t.name for t in theme.available()],
            textvariable=self._theme_var, command=self._on_theme_change,
            width=140,
        )
        ...
        self._canvas = tk.Canvas(
            ...
            highlightthickness=0, bd=0,
            xscrollcommand=self._hscroll.set, yscrollcommand=self._vscroll.set,
        )
    ...
    def _build_right_sidebar(self):
        ...
        self._right_tabs = _SidebarTabBar(
            self._right_frame, ['Properties', 'Actions'],
    ...
    def _build_properties_page(self):
        ...
        self._props_inner.bind(
            '<Configure>',
            lambda e: self._props_canvas.config(
        ...
        self._no_selection_lbl = tk.Label(
            ...
            bg=theme.BG_PANEL, fg=theme.FG_SECONDARY,
            font=theme.LABEL_FONT_SMALL, justify=tk.CENTER,
        )
    ...
    def _build_actions_page(self):
        ...
        ULabel(self._actions_page, text='FORM ACTIONS',
               bg=theme.BG_PANEL, fg=theme.FG_SECONDARY,
               font=('Arial', 8, 'bold')).pack(anchor=tk.W, padx=12, pady=(12, 4))
        ...
        for label, cmd in actions:
            btn = UButton(self._actions_page, text=label, variant='default',
                          command=cmd)
        ...
        ULabel(self._actions_page, text='SELECTION',
               bg=theme.BG_PANEL, fg=theme.FG_SECONDARY,
               font=('Arial', 8, 'bold')).pack(anchor=tk.W, padx=12, pady=(0, 4))

        sel_actions = [
            ('Rename... (F2)', lambda: self._rename_widget(self._selected_id) if self._selected_id else None),
    ...
    def _create_status_bar(self):
        ...
        self._status_widget = tk.Label(
            self._status_bar, text='No selection',
    ...
    def _add_property_section(self, section_title: str, names):
        ...
        for idx, name in enumerate(names):
            ...
            if name == 'variant':
                widget = UComboBox(body, textvariable=var,
    ...
    def _rename_widget(self, item_id):
        ...
        overlay = UEntry(self._surface, textvariable=var,
                         width=max(8, len(item['name']) + 4))
        overlay.place(
            x=item['x'], y=item['y'],
    ...
    def _load_xml(self, path):
        ...
        if window is not None:
            ...
            self._title_var.set(self._window_title)
            self._surface_width = self._to_int(window.get('width', self._surface_width), self._surface_width)
            self._surface_height = self._to_int(window.get('height', self._surface_height), self._surface_height)

        self._configure_surface()

        for idx, elem in enumerate(window.findall('widget') if window is not None else root.findall('widget'), start=1):
    ...
    def _widget_from_element(self, elem, idx):
        ...
        item = {
            ...
            'width': self._to_int(elem.get('width', DEFAULT_SIZE[wtype][0]), DEFAULT_SIZE[wtype][0]),
            'height': self._to_int(elem.get('height', DEFAULT_SIZE[wtype][1]), DEFAULT_SIZE[wtype][1]),
    ...
    def _generate_python(self):
        ...
        imports = ''
        if used_types:
            imports = 'from Uui.widgets import (\n' + ''.join(f'    {t},\n' for t in used_types) + ')\n'
        lines = [
