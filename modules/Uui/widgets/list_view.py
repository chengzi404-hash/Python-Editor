...
class UListView(tk.Frame):
    def __init__(self, parent, columns: list[str] | None = None,
        ...
        for col_idx, col in enumerate(self._columns):
            lbl = tk.Label(
                self._header_frame, text=col, bg=theme.BG_TITLE,
                fg=theme.FG_PRIMARY, font=theme.LABEL_FONT_BOLD,
                anchor='w', padx=8,
            )
        ...
        self._canvas = tk.Canvas(body_frame, bg=theme.BG_PANEL,
        ...
        self._canvas_window = self._canvas.create_window(
            (0, 0), window=self._items_frame, anchor='nw', tags='inner',
        )
    ...
    def _rebuild(self):
        ...
        for idx, row in enumerate(self._data):
            ...
            for col_idx, col in enumerate(self._columns):
                val = row.get(col, '')
                cell = tk.Label(
                    row_container, text=val, bg=item_bg, fg=theme.FG_PRIMARY,
                    font=theme.LABEL_FONT, anchor='w', padx=8,
                )
