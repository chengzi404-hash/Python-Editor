...
def cmd_runserver(args: argparse.Namespace) -> int:
    ...
    if getattr(args, "http2", False) or getattr(args, "ssl_certfile", ""):
        ...
        if (cert or key) and not (cert and key):
            print(
                "  ! both --ssl-cert and --ssl-key are required for HTTPS",
        ...
        if (not cert) and getattr(args, "http2", False):
            ...
            try:
                ...
                cert, key = ensure_dev_cert(
                    cn=host or "localhost", cert_path="cert.pem", key_path="key.pem"
                )
                print(f"  + auto-generated dev cert: {cert}, {key}")
            except Exception as exc:
                print(f"  ! could not generate dev cert: {exc}")
                return 1
        run_http2(host=host, port=port, settings=args.settings, ssl_certfile=cert, ssl_keyfile=key)
        return 0
...
def cmd_createsuperuser(args: argparse.Namespace) -> int:
    ...
    try:
        ...
        u = User(
            username=username, email=email, password_hash="!", is_staff=True, is_superuser=True
        )
...
def build_parser(prog: str = "uui-web") -> argparse.ArgumentParser:
    ...
    p_run.add_argument(
        "addr", nargs="?", default="127.0.0.1:8000", help="host:port (default 127.0.0.1:8000)"
    )
    p_run.add_argument("--settings", default=None, help="settings module path")
    p_run.add_argument(
        "--http2", action="store_true", help="enable HTTP/2 (h2c cleartext or via TLS+ALPN)"
    )
    p_run.add_argument(
        "--ssl-cert",
        dest="ssl_certfile",
    ...
    p_run.add_argument(
        "--ssl-key", dest="ssl_keyfile", default="", help="path to TLS private key (PEM)"
    )
    ...
    p_serve.add_argument("--http2", action="store_true", help="enable HTTP/2 (TLS+ALPN or h2c)")
    p_serve.add_argument(
        "--ssl-cert", dest="ssl_certfile", default="", help="path to TLS certificate (PEM)"
    )
    p_serve.add_argument(
        "--ssl-key", dest="ssl_keyfile", default="", help="path to TLS private key (PEM)"
    )
