...
class UTabView(tk.Frame):
    ...
    def add_tab(self, tab_id: str, label: str) -> tk.Frame:
        ...
        for w in (btn, lbl):
            w.bind("<Button-1>", lambda e: self.select(tab_id))
            w.bind("<Enter>", lambda e: self._on_tab_enter(btn, lbl))
            w.bind("<Leave>", lambda e: self._on_tab_leave(btn, lbl, tab_id))
    ...
    def select(self, tab_id: str):
        ...
        if self._active:
            ...
            prev_lbl: tk.Label = prev_btn.winfo_children()[0]  # type: ignore[assignment]
            prev_lbl.config(fg=theme.FG_SECONDARY)
            self._tabs[self._active].pack_forget()
        self._active = tab_id
        btn = self._buttons[tab_id]
        lbl: tk.Label = btn.winfo_children()[0]  # type: ignore[assignment]
